...
import { BlogsDetail } from "./BlogsDetailsSection";

type BlogsDetailseditSectionProps = {
  id: string;
...
export default function BlogDetailsEditSecction({
  id,
  token,
  getCurrentUser,
  previewImageUrl,
  setPreviewImageUrl,
}: BlogsDetailseditSectionProps) {
  const [detailCourse, setDetailsCourse] = useState<BlogsDetail | null>(null);
  ...
  useEffect(() => {
    async function fetchDetailsCourse() {
      try {
        ...
        setDetailsFormData({
          blogDescription: detailscoursedata.blogDescription,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    detailsFormDataToSend.append("blogDescription", detailsFormData.blogDescription);
    ...
    if (detailsFormData.imageFile) {
      ...
      try {
        await axiosInstance.patch(`/blogs/${id}`, detailsFormDataToSend, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "multipart/form-data",
          },
